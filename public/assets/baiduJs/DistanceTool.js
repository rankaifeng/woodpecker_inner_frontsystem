...
(function() {
        ...
        BMapLib.DistanceTool = function(map, opts) {
            ...
            this._opts = baidu.extend(
                baidu.extend(this._opts || {}, {
                     ...
                     * @type {String}
                     */
                    cursor: "assets/images/ruler.cur", // "http://api.map.baidu.com/images/ruler.cur",
     ...
     * 清空所有测量
     */
    DistanceTool.prototype.clearAllMeasure = function() {
        for (var index = 0; index < this._disObjList.length; index++) {
            var disObj = this._disObjList[index];
            // 关闭本次测距，清除相关存储和变量
            for (var i = 0, l = disObj.points.length; i < l; i++) {
                disObj.points[i].disLabel.remove();
                disObj.points[i].disLabel = null;
